...
<!DOCTYPE html>
	<html lang="pt-BR">
		<head>
			...
			<!-- Título -->
			<title>ADM | <?php echo $vetor[8]; ?> Professor</title>
		</head>
		<body>
			<header>
				<nav class="verdeIF">
					<div class="nav-wrapper black-text">
						...
						<ul class="brand-logo right hide-on-small-only">
							<li><a href="professores.php" class="center">Professores</a></li>
						</ul>
			...
			<div class="container center">
				<h3 class="texto verdeIFtexto"><?php echo $vetor[8]; ?> Professor</h3>
				<?php 
				...
				<form action="acao.php" method="post" enctype="multipart/form-data">
					<div class="row">
						...
						<div class="input-field col l4 s12 offset-l2">
							<?php
								if ($vetor[7] == 'cadastrar') {
									geraSelectMultiple("Disciplinas", "disciplina", 0, 0, "nome", "disciplina");
								}
								else{;
									geraSelectMultiple("Disciplinas", "disciplina", DisciplinaP(isset($_GET['codigo']) ? $_GET['codigo'] : 0), 0, "nome", "disciplina");
								}
							?>
						</div>
						<div class="input-field col l4 s12">
							<?php 
								if ($vetor[7] == 'cadastrar') {
									geraSelectMultipleTurma("Turmas", "turma", 0, 0, "ano", "turma");
								}
								else{
									geraSelectMultipleTurma("Turmas", "turma", TurmaP(isset($_GET['codigo']) ? $_GET['codigo'] : 0), 0, "ano", "turma");
								}
							?>
						</div>
						...
						<div class="input-field col l6 offset-l3 m6 s12">
							<button class="waves-effect waves-light btn verdeIF white-text" type="submit" value="<?php echo $vetor[7] ?>.0" name="acao">
								<?php echo $vetor[7] ?>
							</button>
						</div>
						<div class="input-field col l6 offset-l3 m6 s12">
							<?php
								if ($vetor[7] == 'alterar') {
							?>
							<button class="waves-effect waves-light btn verdeIF white-text" type="submit" value="alterarOcupacao.1" name="acao">
								Tornar Admin
							</button>
							<?php
								}
							...
							<input type="hidden" name="pagina" value="professores.php">
							<input type="hidden" name="tabela" value="usuario">
							<input type="hidden" name="ocupacao" value=2>
						</div>
					</div>
				</form>
			</div>
